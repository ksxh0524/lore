...
import { MacShell, MacMenuBar, MacDock, MacWindow } from './MacShell';
...
function FinderApp() {
  return (
    <div style={{ display: 'flex', height: '100%', fontFamily: '-apple-system, SF Pro Text, system-ui, sans-serif' }}>
      <div style={{ width: 180, background: 'rgba(40,40,42,0.6)', padding: '12px 0', borderRight: '0.5px solid rgba(255,255,255,0.06)' }}>
        {['收藏', '桌面', '文稿', '下载', '应用程序'].map((item, i) => (
          <div key={item} style={{
            ...
            borderRadius: 6, margin: '1px 8px', cursor: 'pointer',
          }}>{item}</div>
        ))}
      </div>
      <div style={{ flex: 1, padding: 16, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, alignContent: 'start' }}>
        {['项目报告.pdf', '工作计划.docx', '会议记录.md', '预算表.xlsx', '照片备份', '代码仓库'].map((f) => (
          <div key={f} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, padding: 8, borderRadius: 8, cursor: 'pointer' }}>
            <div style={{ fontSize: 40 }}>📄</div>
            <span style={{ fontSize: 11, color: ios26.colors.text.secondary, textAlign: 'center' }}>{f}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', fontFamily: '-apple-system, SF Pro Text, system-ui, sans-serif' }}>
      <div style={{
        ...
        <div style={{
          ...
          background: 'rgba(120,120,128,0.24)', fontSize: 13,
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12, padding: 32 }}>
        ...
        <div style={{ fontSize: ios26.typography.subhead.size, color: ios26.colors.text.tertiary }}>在这里浏览世界信息、事件、Agent 数据</div>
      </div>
    </div>
  );
}
...
function MonitorApp() {
  return (
    <div style={{ padding: 16, fontFamily: '-apple-system, SF Pro Text, system-ui, sans-serif', height: '100%', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
        {[
          ...
          <div key={stat.label} style={{
            ...
            <div style={{ fontSize: 11, color: ios26.colors.text.tertiary, marginBottom: 4 }}>{stat.label}</div>
    ...
    <MacShell>
      <div style={{ width: '100%', height: '100%', position: 'relative', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{
          ...
          {app && AppComp ? (
            <MacWindow title={app.title} onClose={() => setActiveApp(null)} width={680} height={380}>
              <AppComp />
            </MacWindow>
          ) : (
            <div style={{ textAlign: 'center', color: ios26.colors.text.tertiary }}>
              <div style={{ fontSize: 64, marginBottom: 12 }}>🖥️</div>
              <div style={{ fontSize: ios26.typography.title3.size, fontWeight: 500 }}>点击 Dock 打开应用</div>
            </div>
          )}
